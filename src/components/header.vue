<template>
  <v-toolbar v-if="this.$store.getters.inLogin" app class="bg-indigo-darken-3">
    ...
    <v-spacer></v-spacer>
    <router-link v-if="this.$store.getters.getUser.type === 'client'" class="text-decoration-none text-white" :to="`/user/${this.$store.getters.getUser.id}`"><v-btn>Profile</v-btn></router-link>
    <router-link v-else class="text-decoration-none text-white" :to="`/lawyer/${this.$store.getters.getUser.id}`"><v-btn>Profile</v-btn></router-link>
  ...
  <v-navigation-drawer v-if="this.$store.getters.getUser.type === 'lawyer'" v-model="barra" class="bg-indigo-darken-3" >
  ...
  <v-navigation-drawer v-else v-model="barra" class="bg-indigo-darken-3" >
    <v-list temporary class="d-flex justify-center items pa-0 bg-indigo-darken-3">
      <v-list-item-group v-model="barra_grupo" class="bg-indigo-darken-3 pt-16">
        <v-list-item>
          <v-list-item-title><router-link class="text-decoration-none text-white" to="/notifications" @click=" barra  = !barra"><v-btn class="bg-indigo-darken-3">Notifications</v-btn></router-link></v-list-item-title>
